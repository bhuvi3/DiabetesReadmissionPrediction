...
out = open('../results/All_plot30_final_10.txt','w')
pp = PdfPages('../results/All_plot30_final_new_10.pdf')
...
for i in range(n_classes):
    if mapping[i] == '<30':
        ...
        break
        #remove this break if un-commenting above line

plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('Recall')
plt.ylabel('Precision')
plt.title('Precision-Recall curve for <30 versur rest')
###########
#1
precision = dict()
recall = dict()
...
for i in range(n_classes):
    if mapping[i] == '<30':
        plt.plot(recall[i], precision[i], label='RandomForest-PR for {' + mapping[i] + '}' + ' (PR_area = {1:0.2f})'.format(i, average_precision[i]))
#
#2
precision = dict()
recall = dict()
...
for i in range(n_classes):
    if mapping[i] == '<30':
        plt.plot(recall[i], precision[i], label='AdaBoost-PR for {' + mapping[i] + '}' + ' (PR_area = {1:0.2f})'.format(i, average_precision[i]))
#
#3
precision = dict()
recall = dict()
...
for i in range(n_classes):
    if mapping[i] == '<30':
        plt.plot(recall[i], precision[i], label='NaiveBayes-PR for {' + mapping[i] + '}' + ' (PR_area = {1:0.2f})'.format(i, average_precision[i]))
#
"""#4
precision = dict()
recall = dict()
...
for i in range(n_classes):
    if mapping[i] == '<30':
        plt.plot(fpr[i], tpr[i], label='RandomForest-ROC for {' + mapping[i] + '}' + ' (ROC_area = {1:0.2f})'.format(i, ROC_area[i]))
...
for i in range(n_classes):
    if mapping[i] == '<30':
        plt.plot(fpr[i], tpr[i], label='AdaBoost-ROC for {' + mapping[i] + '}' + ' (ROC_area = {1:0.2f})'.format(i, ROC_area[i]))
        [y1,mi1]=eer(fpr[i],tpr[i])    
        plt.plot(fpr[i][mi1],y1[mi1],'o')
...
for i in range(n_classes):
    if mapping[i] == '<30':
        plt.plot(fpr[i], tpr[i], label='NaiveBayes-ROC for {' + mapping[i] + '}' + ' (ROC_area = {1:0.2f})'.format(i, ROC_area[i]))
        [y1,mi1]=eer(fpr[i],tpr[i])    
        plt.plot(fpr[i][mi1],y1[mi1],'o')
...
for i in range(n_classes):
    if mapping[i] == '<30':
        plt.plot(fpr[i], tpr[i], label='DT-ROC for {' + mapping[i] + '}' + ' (ROC_area = {1:0.2f})'.format(i, ROC_area[i]))
        [y1,mi1]=eer(fpr[i],tpr[i])    
        plt.plot(fpr[i][mi1],y1[mi1],'o')
...
for i in range(n_classes):
    if mapping[i] == '<30':
        #plt.plot(fpr[i], tpr[i], label='ALL Probability Summation-ROC for {' + mapping[i] + '}' + ' (ROC_area = {1:0.2f})'.format(i, ROC_area[i]))
        [y1,mi1]=eer(fpr[i],tpr[i])    
        plt.plot(fpr[i][mi1],y1[mi1],'o')
...
plt.title('ROC curve for <30 versus rest')
